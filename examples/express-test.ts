 ...
 * Run with: npx ts-node examples/express-test.ts
 */
...
async function main() {
  ...
  // Test routes
  app.get('/', (req, res) => {
    (req as any).log.info('Home page accessed');
    res.json({ message: 'Hello from Express!' });
  });

  app.get('/users', (req, res) => {
    (req as any).log.info('Fetching users');
    (req as any).log.debug('Query params', { query: req.query });
    res.json({ users: [{ id: 1, name: 'John' }, { id: 2, name: 'Jane' }] });
  });

  app.get('/error', (req, res) => {
    (req as any).log.error('Something went wrong!', { code: 'ERR_TEST' });
